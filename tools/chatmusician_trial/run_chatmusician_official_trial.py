...
DEFAULT_TEMPLATE = PROJECT_ROOT / "configs" / "text2midi" / "prompts" / "chatmusician_trial_official_like.txt"
DEFAULT_QUESTION = "What key and meter does this piece most likely use? Answer in one short sentence."
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run ChatMusician with an official-like Human/Assistant prompt.")
...
def build_prompt(template: str, abc_text: str, question: str) -> str:
    return template.replace("{{ABC}}", abc_text.strip()).replace("{{QUESTION}}", question.strip())
...
def main():
    ...
    prompt = build_prompt(template, cleaned_abc, args.question)
    ...
    (output_dir / "chatmusician_official_trial_result.json").write_text(

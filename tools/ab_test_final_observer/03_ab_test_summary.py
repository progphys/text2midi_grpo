...
def parse_args():
    parser = argparse.ArgumentParser(description="Aggregate scored A/B samples into prompt-level statistics.")
...
def main():
    args = parse_args()
    scored_path = resolve_path(args.scored_json)
    output_summary = resolve_path(args.output_summary) if args.output_summary else scored_path.parent / "ab_summary.json"
    output_prompt_level = (
        resolve_path(args.output_prompt_level) if args.output_prompt_level else scored_path.parent / "ab_prompt_level.json"
    )

    rows = load_json(scored_path)
    grouped: dict[int, dict[str, list[dict]]] = defaultdict(lambda: {"base": [], "final": []})
    for row in rows:
        grouped[int(row["prompt_index"])][row["model_name"]].append(row)
    ...
    for prompt_index in sorted(grouped):
        base_rows = [r for r in grouped[prompt_index]["base"] if r.get("final_observer_raw") is not None]
        final_rows = [r for r in grouped[prompt_index]["final"] if r.get("final_observer_raw") is not None]
        base_scores = [float(r["final_observer_raw"]) for r in base_rows]
        final_scores = [float(r["final_observer_raw"]) for r in final_rows]

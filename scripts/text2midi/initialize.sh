...
MODEL_WEIGHTS_DIR="$MODEL_DIR/weight_models/text2midi"
TOKENIZER_DIR="$MODEL_DIR/weight_models/flan-t5-base"
...
resolve_python_bin() {
    ...
    for candidate in "${TEXT2MIDI_PYTHON:-}" python3.10 python3 python; do
        ...
        if resolved=$("$candidate" -c 'import sys, venv; print(sys.executable)' 2>/dev/null); then
            echo "$resolved"
            return 0
        fi
    done
...
if ! PYTHON_BIN="$(resolve_python_bin)"; then
    echo "Python 3 with venv support is required but not found."
    exit 1
fi
...
if [ ! -d "$VENV_DIR" ]; then
    ...
    if ! "$PYTHON_BIN" -m venv "$VENV_DIR"; then
        ...
        echo "This usually means python3.10-venv / ensurepip is not installed."
        echo "Falling back to system/current Python installation."
        echo ""
        rm -rf "$VENV_DIR"
        exec bash "$PROJECT_ROOT/scripts/text2midi/initialize_system.sh"
    fi
else
...
# 4. Download weights from HuggingFace
...
echo ""
echo "Done! Activate venv with:"
echo "  source .venv/bin/activate"
echo ""
echo "Then try:"
echo "  ./run.sh infer --caption \"A calm piano solo in E minor\""

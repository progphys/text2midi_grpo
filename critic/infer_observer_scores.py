...
from core.critics.observer.paths import SPECS_DIR
from core.critics.observer.support.theory_helpers import build_theory_context
from core.critics.observer.data_pipeline import build_observer_graph, build_observer_song_record
from core.critics.observer.model import ObserverGNN
from core.critics.observer.schema import OBSERVER_EDGE_TYPES, OBSERVER_NUM_FIELDS, build_observer_vocab_sizes
...
def _project_root() -> Path:
    ...
    for candidate in [current.parent, *current.parents]:
        if (candidate / "src" / "core" / "critics" / "observer" / "assets" / "specs" / "spec_global.json").exists():
            return candidate
    raise ObserverInferenceError("Could not locate project root containing metadata/specs/spec_global.json")
...
def _load_spec_global() -> dict[str, Any]:
    path = SPECS_DIR / "spec_global.json"
